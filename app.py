...
ERA_INFO = {
    "Pionir": {
        "emoji": "🎷", "rentang": "~1920–1969", "warna": "#C0A060",
        "desc": "Era perintisan musik modern. Para legenda membangun fondasi dari nol.",
        "tokoh": "The Beatles · Bob Dylan · Miles Davis"
    },
    "Old School": {
        "emoji": "🎸", "rentang": "1970–1984", "warna": "#E07040",
        "desc": "Era kejayaan rock, soul, dan funk. Ikonik dan tidak lekang waktu.",
        "tokoh": "Led Zeppelin · Pink Floyd · Stevie Wonder"
    },
    "Mid High School": {
        "emoji": "📼", "rentang": "1985–1999", "warna": "#5080D0",
        "desc": "Era alternatif dan grunge. Penuh eksperimen dan keberanian.",
        "tokoh": "Nirvana · Radiohead · Nas · Björk"
    },
    "New School": {
        "emoji": "💿", "rentang": "2000–2012", "warna": "#40B080",
        "desc": "Era digital awal. Gerakan indie mulai bersinar luas.",
        "tokoh": "My Chemical Romance · Kanye West · Sufjan Stevens"
    },
    "New New School": {
        "emoji": "📱", "rentang": "2013–kini", "warna": "#A050E0",
        "desc": "Era streaming. Musik semakin beragam dan mudah dijangkau.",
        "tokoh": "Kendrick Lamar · Frank Ocean · Laufey"
    },
}

ACC_INFO = {
    "Niche": {
        "emoji": "🔬", "warna": "#3050A0", "singkat": "Sangat Niche",
        "desc": "Selera Anda sangat spesifik dan jarang dikenal khalayak umum."
    },
    "Elitis": {
        "emoji": "🎓", "warna": "#6040B0", "singkat": "Elitis",
        "desc": "Anda cenderung menyukai musik underground bermutu tinggi."
    },
    "Tidak Basic": {
        "emoji": "🎸", "warna": "#208060", "singkat": "Tidak Basic",
        "desc": "Selera Anda berada di luar arus utama, namun masih terjangkau."
    },
    "Agak Lumayan Basic": {
        "emoji": "🎧", "warna": "#808020", "singkat": "Cukup Populer",
        "desc": "Anda menyukai musik yang cukup dikenal tanpa terlalu mainstream."
    },
    "Basic": {
        "emoji": "📻", "warna": "#A04020", "singkat": "Sangat Populer",
        "desc": "Anda menyukai musik yang sangat populer dan dikenal luas."
    },
}

MOOD_DESC = {
    "melancholic": "Bernuansa sedih namun indah dan menyentuh.",
    "atmospheric": "Membangun suasana dan ruang yang mendalam.",
    "introspective": "Mengajak merefleksikan diri ke dalam.",
    "anxious": "Bernuansa gelisah dan penuh ketegangan.",
    "cold": "Terasa dingin dan berjarak, namun tetap menarik.",
    "energetic": "Penuh semangat dan energi.",
    "dark": "Mengandung unsur kegelapan yang intens.",
    "epic": "Berskala besar dan terasa megah.",
    "psychedelic": "Membuka persepsi dengan cara yang unik.",
    "hypnotic": "Memiliki daya hipnotis yang kuat.",
    "lonely": "Terasa sunyi dan penuh kerinduan.",
    "romantic": "Menghadirkan kehangatan cinta.",
...
def label(text):
    ...
    st.markdown(
        f"<p style='font-family:DM Mono,monospace;font-size:0.7rem;color:#555;"
        f"letter-spacing:0.18em;text-transform:uppercase;margin-bottom:0.5rem;'>"
        f"{text}</p>",
        unsafe_allow_html=True
    )
...
def card(content, warna="#1e1e1e", border_left=None):
    ...
    st.markdown(
        f"<div style='background:#0f0f0f;border:1px solid {warna}33;{bl}"
        f"border-radius:4px;padding:1.3rem;'>{content}</div>",
        unsafe_allow_html=True
    )
...
def divider(title):
    ...
    st.markdown(
        f"<div style='margin:2.5rem 0 1.3rem;padding-top:1.8rem;"
        f"border-top:1px solid #161616;'>"
        f"<p style='font-family:DM Mono,monospace;font-size:0.68rem;color:#444;"
        f"letter-spacing:0.22em;text-transform:uppercase;margin:0;'>{title}</p>"
        f"</div>",
        unsafe_allow_html=True
    )
...
def get_era(dfc):
    ...
    p = model_era.predict(
        dfc[["year", "avg_rating", "log_rating_count", "log_review_count", "accessibility"]]
    )
...
def get_acc(dfc):
    ...
    p = model_akses.predict(
        dfc[["accessibility", "avg_rating", "log_rating_count", "log_review_count"]]
    )
    return Counter(p).most_common(1)[0][0]
...
def get_mood(dfc):
    ...
    semua = [
        k.strip()
        for d in dfc["descriptors"].dropna()
        for k in d.split(",")
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "color:#f0ede8;letter-spacing:-0.03em;line-height:1;margin:1rem 0 0.2rem;\">"
        "REQ<sup style='font-size:0.3em;vertical-align:super;color:#e8d5a3;'>✦</sup></h1>",
        unsafe_allow_html=True
    )
    st.markdown(
        "<p style='font-family:DM Mono,monospace;font-size:0.62rem;color:#333;"
        "letter-spacing:0.16em;margin:0 0 1.2rem;'>MUSIC MACHINE LEARNING</p>",
        unsafe_allow_html=True
    )
    st.markdown(
        "<hr style='border:0;border-top:1px solid #1a1a1a;margin:0 0 1.2rem;'>",
        unsafe_allow_html=True
    )
    ...
    for ikon, judul, ket in menu_items:
        st.markdown(
            f"<div style='padding:0.6rem 0.8rem;margin-bottom:0.3rem;border-radius:3px;"
            f"border:1px solid #161616;background:#0c0c0c;'>"
            f"<p style='font-family:DM Mono,monospace;font-size:0.65rem;color:#e8d5a3;"
            f"margin:0 0 0.15rem;'>{ikon} {judul}</p>"
            f"<p style='font-size:0.82rem;color:#444;margin:0;'>{ket}</p>"
            f"</div>",
            unsafe_allow_html=True
        )

    st.markdown(
        "<hr style='border:0;border-top:1px solid #1a1a1a;margin:1rem 0;'>",
        unsafe_allow_html=True
    )
    ...
    stats = [
        ("Total Album", f"{len(df):,}"),
        ("Total Artis", f"{df['artist_name'].nunique():,}"),
        ("Rentang Tahun", f"{int(df['year'].min())}–{int(df['year'].max())}"),
        ("Rata-rata Rating", f"{df['avg_rating'].mean():.2f}"),
    ]
    for nama, nilai in stats:
        st.markdown(
            f"<div style='display:flex;justify-content:space-between;padding:0.35rem 0;"
            f"border-bottom:1px solid #111;'>"
            f"<span style='font-size:0.82rem;color:#444;'>{nama}</span>"
            f"<span style='font-family:DM Mono,monospace;font-size:0.82rem;"
            f"color:#e8d5a3;'>{nilai}</span>"
            f"</div>",
            unsafe_allow_html=True
        )
    ...
    label("Akurasi Model")
    akurasi = [
        ("Model Era — Random Forest", "94.71 %"),
        ("Model Aksesibilitas — Random Forest", "97.12 %"),
    ]
    for nm, val in akurasi:
        st.markdown(
            f"<div style='padding:0.5rem 0.7rem;margin-bottom:0.3rem;border-radius:3px;"
            f"border:1px solid #161616;background:#0c0c0c;'>"
            f"<p style='font-family:DM Mono,monospace;font-size:0.6rem;color:#555;"
            f"margin:0 0 0.1rem;'>{nm}</p>"
            f"<p style='font-family:DM Mono,monospace;font-size:0.75rem;"
            f"color:#e8d5a3;margin:0;'>{val}</p>"
            f"</div>",
            unsafe_allow_html=True
        )
    ...
    st.markdown(
        "<p style='font-family:DM Mono,monospace;font-size:0.55rem;color:#222;"
        "letter-spacing:0.12em;line-height:2;border-top:1px solid #1a1a1a;"
        "padding-top:1rem;'>DATA · RateYourMusic<br>MODEL · scikit-learn<br>"
        "UI · Streamlit</p>",
        unsafe_allow_html=True
    )
...
with Analisis:
    st.markdown(
        ...
        "color:#f0ede8;margin-bottom:0.2rem;\">Analisis &amp; Prediksi</h1>",
        unsafe_allow_html=True
    )
    st.markdown(
        "<p style='font-family:DM Mono,monospace;font-size:0.7rem;color:#444;"
        "letter-spacing:0.15em;margin-bottom:2rem;'>PILIH ALBUM ATAU ARTIS — "
        "SISTEM AKAN MENGANALISIS SELERA MUSIK ANDA</p>",
        unsafe_allow_html=True
    )
    ...
    if run:
        if not sel_albums and not sel_artists:
            ...
            if dfc.empty:
                ...
                st.markdown(
                    f"<p style='font-size:0.9rem;color:#555;font-style:italic;"
                    f"margin-top:1rem;'>Analisis didasarkan pada {len(dfc)} album "
                    f"yang ditemukan dalam dataset.</p>",
                    unsafe_allow_html=True
                )
                ...
                with ca:
                    skala = "".join(
                        f"<div style='flex:1;height:5px;"
                        f"background:{ai['warna'] if lv == acc_fav else '#1e1e1e'};"
                ...
                for i, (mood, count) in enumerate(top_mood):
                    deskripsi = MOOD_DESC.get(mood, 'Deskriptor unik yang jarang ditemukan.')
                    mood_cols[i].markdown(f"""
                    <div style='background:#0f0f0f;border:1px solid #1e1e1e;border-radius:4px;padding:1.2rem;height:100%;'>
                        <p style='font-family:DM Mono,monospace;font-size:0.65rem;color:#e8d5a3;margin:0 0 0.4rem;'>#{i + 1}</p>
                        <h3 style="font-family:'DM Serif Display',serif;font-size:1.3rem;color:#f0ede8;margin:0 0 0.3rem;text-transform:capitalize;">{mood}</h3>
                        <p style='font-family:DM Mono,monospace;font-size:0.62rem;color:#333;margin:0 0 0.6rem;'>muncul {count}x</p>
                        <p style='font-size:0.88rem;color:#555;line-height:1.6;margin:0;'>{deskripsi}</p>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                for i, (_, r) in enumerate(df_rek.iterrows()):
                    deskriptor = r.get("descriptors", "")
                    vibes = (
                        " · ".join(
                            [k.strip() for k in deskriptor.split(",") if k.strip() not in EXCLUDE][:4]
                        )
                        if pd.notna(deskriptor) else ""
                    )
                    ...
                    spot = (
                        ...
                        f'font-size:0.65rem;font-weight:700;padding:0.3rem 0.8rem;'
                ...
                st.dataframe(
                    dfc[[
                        "release_name", "artist_name", "year", "avg_rating",
...
with Introduction:
    st.markdown(
        "<h1 style=\"font-family:'DM Serif Display',serif;font-size:5rem;"
        "color:#f0ede8;letter-spacing:-0.03em;line-height:1;margin:0 0 0.5rem;\">"
        "REQ<sup style='font-size:0.28em;vertical-align:super;color:#e8d5a3;'>✦</sup></h1>",
        unsafe_allow_html=True
    )
    st.markdown(
        "<p style='font-family:DM Mono,monospace;font-size:0.7rem;color:#444;"
        "letter-spacing:0.2em;margin:0 0 2rem;'>SISTEM REKOMENDASI MUSIK "
        "BERBASIS MACHINE LEARNING</p>",
        unsafe_allow_html=True
    )
    st.markdown(
        "<p style='font-size:1rem;color:#666;max-width:560px;line-height:1.9;"
        "margin-bottom:2rem;'>Masukkan album atau artis favorit Anda — REQ akan "
        "memprediksi era musik, tingkat aksesibilitas selera, dan mood dominan Anda, "
        "lalu memberikan rekomendasi album yang paling sesuai.</p>",
        unsafe_allow_html=True
    )
    ...
    fig = px.bar(
        ec, x="era", y="count", template="plotly_dark", color="era",
        color_discrete_map={
            "Pionir": "#C0A060",
            "Old School": "#E07040",
            "Mid High School": "#5080D0",
            "New School": "#40B080",
            "New New School": "#A050E0",
        }
    )
    fig.update_layout(
        paper_bgcolor="rgba(0,0,0,0)",
    ...
    for col, num, judul, isi in langkah:
        col.markdown(f"""
        <div style='background:#0f0f0f;border:1px solid #1e1e1e;border-radius:4px;padding:1.3rem;'>
            <p style='font-family:DM Mono,monospace;font-size:0.55rem;color:#2a2a2a;margin:0 0 0.4rem;'>{num}</p>
            <p style="font-family:'DM Serif Display',serif;font-size:1rem;color:#e8d5a3;margin:0 0 0.4rem;">{judul}</p>
            <p style='font-size:0.88rem;color:#555;line-height:1.6;margin:0;'>{isi}</p>
        </div>
        """, unsafe_allow_html=True)
...
with Dataset:
    st.markdown(
        "<h1 style=\"font-family:'DM Serif Display',serif;font-size:3rem;"
        "color:#f0ede8;margin-bottom:0.2rem;\">Dataset</h1>",
        unsafe_allow_html=True
    )
    st.markdown(
        "<p style='font-family:DM Mono,monospace;font-size:0.7rem;color:#444;"
        "letter-spacing:0.15em;margin-bottom:2rem;'>SUMBER · RATEYOURMUSIC — "
        "5.000+ ALBUM</p>",
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        "<hr style='border:0;border-top:1px solid #1a1a1a;margin:1.5rem 0 1rem;'>",
        unsafe_allow_html=True
    )
    ...
    df_show = df.copy()
    if era_f:
        df_show = df_show[df_show["era"].isin(era_f)]
    if acc_f:
        df_show = df_show[df_show["acc_label"].isin(acc_f)]

    label(f"{len(df_show):,} baris ditemukan")
    st.dataframe(
        df_show[[
            "release_name", "artist_name", "year", "avg_rating", "rating_count",
            "accessibility", "era", "acc_label", "primary_genres"
        ]].head(n_r),
        use_container_width=True,
        height=420,
...
with About_Me:
    st.markdown(
        "<h1 style=\"font-family:'DM Serif Display',serif;font-size:3rem;"
        "color:#f0ede8;margin-bottom:2rem;\">Tentang Saya</h1>",
        unsafe_allow_html=True
    )
    ...
    with cf:
        st.markdown(
            "<div style='border:1px solid #1e1e1e;border-radius:4px;overflow:hidden;'>"
            "<img src='https://raw.githubusercontent.com/MF-KIMPOEL/projek_ML_Musik/main/hafidz.jpeg' "
        ...
        teknologi = ["Python", "Streamlit", "scikit-learn", "pandas", "Plotly", "joblib", "RateYourMusic"]
        badges = " ".join(
            f"<span style='font-family:DM Mono,monospace;font-size:0.65rem;color:#e8d5a3;"
            f"background:#1a1a1a;padding:0.25rem 0.6rem;border-radius:2px;"
            f"border:1px solid #2a2a2a;display:inline-block;margin:0.2rem 0.1rem;'>{t}</span>"
            for t in teknologi
        )
    ...
    with ci:
        ...
        st.markdown(
            ...
            "letter-spacing:0.2em;margin:0 0 2rem;'>SISWA &middot; RPL &middot; "
            "SMKN PURBALINGGA</p>",
            unsafe_allow_html=True
        )
        st.markdown(
            "<p style='font-size:0.95rem;color:#666;line-height:1.9;margin-bottom:1.5rem;'>"
            "Nama saya Rigel Amadeus Volker, siswa SMKN Purbalingga jurusan RPL kelas 11. "
            "Saya menyukai musik dan film — oleh karena itu saya mengembangkan proyek "
            "machine learning ini sebagai bentuk eksplorasi minat tersebut.</p>",
            unsafe_allow_html=True
        )

        label("KONTAK")
        st.markdown(
            "<p style='font-size:0.9rem;color:#555;line-height:2.4;'>"
            "&#128231; rigel123@gmail.com<br>"
            "&#128025; github.com/rigelgithub<br>"
            "&#127925; last.fm/gapunya/gapunya123</p>",
            unsafe_allow_html=True
        )
        st.markdown(
            "<hr style='border:0;border-top:1px solid #1a1a1a;margin:1.2rem 0;'>",
            unsafe_allow_html=True
        )

        label("TENTANG PROYEK")
        st.markdown(
            "<p style='font-size:0.95rem;color:#666;line-height:1.9;'>"
            "REQ adalah sistem rekomendasi album berbasis Machine Learning menggunakan "
            "dataset dari RateYourMusic. Model mengklasifikasikan era dan aksesibilitas "
            "selera musik pengguna, lalu merekomendasikan album berdasarkan mood dan era "
...
with Kode_Proyek:
    st.markdown(
        "<h1 style=\"font-family:'DM Serif Display',serif;font-size:3rem;"
        "color:#f0ede8;margin-bottom:0.2rem;\">Kode Proyek</h1>",
        unsafe_allow_html=True
    )
    st.markdown(
        "<p style='font-family:DM Mono,monospace;font-size:0.7rem;color:#444;"
        "letter-spacing:0.15em;margin-bottom:2rem;'>proyek reyal.ipynb — "
        "Kode Model ML</p>",
        unsafe_allow_html=True
    )
    ...
    if not os.path.exists(nb_path):
        ...
        for i, cell in enumerate(nb.cells):
            ...
            st.markdown(f"""
            <div style="background:#1e2e22;border-left:3px solid #2fa05a;padding:6px 14px;border-radius:6px;margin-bottom:4px;">
            ...
            if tipe == "markdown":
                ...
                for output in cell.get("outputs", []):
                    ...
                    if output.output_type == "stream":
                        ...
                        st.markdown(
                            f'<div style="background:#0f1715;border-radius:6px;padding:10px 14px;'
                            f'font-family:monospace;font-size:0.82rem;color:#a8d5b5;'
                            f'white-space:pre-wrap;margin-top:4px;">{teks}</div>',
                            unsafe_allow_html=True
                        )
